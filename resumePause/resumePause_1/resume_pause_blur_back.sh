...
echo "start resume_pause_blue_back $dev"
adb -s $dev root
...
#open mtklog
sleep 5
adb -s $dev shell am start start com.ape.logger/.ui.LoggerActivity
#mtklog_permission
...
#LOG file
firstDir=$(pwd)/resume_pause_blur_back
if [[ ! -d "$firstDir" ]]; then
	mkdir $firstDir
fi
...
cameraAppMemroyFile=$scondDir/resumepause_cameraApp_memory.txt  #操作app日志存放路径     
cameraServerMemroyFile=$scondDir/resumepause_cameraServer_memory.txt  #操作app日志存放路径
cameraHalServerMemroyFile=$scondDir/resumepause_camerHalServer_memory.txt  #操作app日志存放路径  
...
adb -s $dev shell input tap $blx $bly 
...
echo " camera app pid is $pid"  >> $log
...
sleep 2
adb -s $dev shell input tap $hx $hy
val=`expr $i % 50`
if [ $val == 0 ]
then
