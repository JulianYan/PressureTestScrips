...
echo "start capture_beauty_front $dev"
adb -s $dev root
...
#open mtklog
sleep 5
adb -s $dev shell am start com.journey.logger/.ui.LoggerActivity
#mtklog_permission
...
#LOG file
firstDir=$(pwd)/capture_beauty_front
if [[ ! -d "$firstDir" ]]; then
	mkdir $firstDir
fi
...
sleep 2
adb -s $dev shell input tap $bex $bey 
