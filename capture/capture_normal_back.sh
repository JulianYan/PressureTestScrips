...
echo "start capture_normal_back $dev"
adb -s $dev root
adb -s $dev shell rm sdcard/DCIM/Camera/*

#KILL process
adb -s $dev shell pkill com.nothing.camera
...
#LOG file
firstDir=$(pwd)/capture_normal_back
if [[ ! -d "$firstDir" ]]; then
	mkdir $firstDir
fi
scondDir=$firstDir/$(date +%Y-%m-%d-%H-%M-%S)
mkdir $scondDir
cameraAppMemroyFile=$scondDir/capture_cameraApp_memory.txt  #操作app日志存放路径     
cameraServerMemroyFile=$scondDir/capture_cameraServer_memory.txt  #操作app日志存放路径
cameraHalServerMemroyFile=$scondDir/capture_camerHalServer_memory.txt  #操作app日志存放路径  

#open camera
adb -s $dev shell am start com.nothing.camera/com.nothing.camera.activity.CameraActivity
...
pid=$(adb -s  $dev shell ps -ef | grep "com.nothing.camera" | grep -v grep | awk '{print $2}')
...
for (( i=0; i<=$total; i++ ))
do
adb -s $dev shell input tap $capx $capy
...
adb -s $dev shell input tap $backx $backy
