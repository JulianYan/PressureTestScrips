...
echo "start change_mode_blue_to_video $dev"
adb -s $dev root
...
#open mtklog
sleep 5
adb -s $dev shell am com.ape.logger/.ui.LoggerActivity
#mtklog_permission
...
#LOG file
firstDir=$(pwd)/change_mode_blur_to_video
if [[ ! -d "$firstDir" ]]; then
	mkdir $firstDir
fi
...
cameraAppMemroyFile=$scondDir/changemode_cameraApp_memory.txt  #操作app日志存放路径     
cameraServerMemroyFile=$scondDir/changemode_cameraServer_memory.txt  #操作app日志存放路径
cameraHalServerMemroyFile=$scondDir/changemode_camerHalServer_memory.txt  #操作app日志存放路径  
...
sleep 1
adb -s $dev shell input tap $vx $vy
val=`expr $i % 50`
if [ $val == 0 ] 
then
