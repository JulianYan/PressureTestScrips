...
echo "start long_captrue_back $dev"
adb -s $dev root
...
#LOG file
firstDir=$(pwd)/long_captrue_back
...
cameraAppMemroyFile=$scondDir/longcapture_cameraApp_memory.txt  #操作app日志存放路径     
cameraServerMemroyFile=$scondDir/longcapture_cameraServer_memory.txt  #操作app日志存放路径
cameraHalServerMemroyFile=$scondDir/longcapture_camerHalServer_memory.txt  #操作app日志存放路径  
...
for (( i=0; i<=$total; i++ ))
do
adb -s $dev shell input swipe $capx $capy $capx $capy 5000 
